...
class FFT {
	...
	void compute_hammer();
	void compute_melmat(uint16_t num_mel_bands, float freq_min, float freq_max, uint16_t num_fft_bands,
...
void FFT::compute_melmat(uint16_t num_mel_bands, float freq_min, float freq_max, uint16_t num_fft_bands,
			 uint16_t sample_rate)
{
...
void FFT::t2mel(float *y_data, float *mel_data)
{
	...
	for (jj = 1; jj < _num_samples; jj++) {
		if (y_data[jj] < minData)
			minData = y_data[jj];
		if (y_data[jj] > maxData)
			maxData = y_data[jj];
		if (maxData - minData > _min_volume_threshold)
			break;
	}
	if (jj == _num_samples) {
